## Hi there 👋

<!--
**CaptainRhett/CaptainRhett** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

# Hi, I'm Wuhuang Yao 👋

<p align="left">
  <b>MSc Student in Cyberspace Security</b><br>
  Beijing Institute of Technology
</p>

<p align="left">
  <a href="mailto:captainrhettyao@gmail.com">Email</a> ·
  <a href="https://github.com/CaptainRhett">GitHub</a> ·
  <!-- <a href="#">Google Scholar</a> · -->
  <!-- <a href="#">CV</a> -->
</p>

---

## About Me

I am currently pursuing a master's degree at the School of Cyberspace Science and Technology, Beijing Institute of Technology.

My research interests mainly focus on cybersecurity, softwear security, and trustworthy machine learning systems. I am also interested in practical security problems in real-world systems.

---

## Research Interests

- Privacy-preserving Machine Learning
- Data Security and Privacy
- AI Agent Security
- System and Firmware Security

---

## Selected Publications

- **JLeaks: A Featured Resource Leak Repository Collected From Hundreds of Open-Source Java Projects**  
  *ICSE 2024*  
  We present the largest verified resource leak repository, containing 1,094 cases with source code, patches, and bytecode, and show that exception handling is the primary cause while existing tools and LLMs still struggle to detect such leaks effectively.


<!-- - **More publications coming soon.** -->

---

## Selected Projects

### Security Testing for AI Agents

Exploring policy compliance, stateful testing, and tool-use risks in autonomous AI agent systems.

**Keywords:** AI Agent Security, Tool-use Agent, Policy Compliance, Security Testing

### Redfish API Recovery and BMC Firmware Security

Studying closed-source BMC firmware and Redfish API behavior for cross-layer protocol specification recovery and stateful security testing.

**Keywords:** BMC Firmware, Redfish, API Recovery, Fuzzing, System Security

---

## Technical Skills

<p align="left">
  <img src="https://img.shields.io/badge/Python-333333?style=flat&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/C/C++-333333?style=flat&logo=cplusplus&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-333333?style=flat&logo=linux&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-333333?style=flat&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/LaTeX-333333?style=flat&logo=latex&logoColor=white" />
  <img src="https://img.shields.io/badge/PyTorch-333333?style=flat&logo=pytorch&logoColor=white" />
</p>


---

## GitHub Stats

<p align="left">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=CaptainRhett&show_icons=true&theme=default&hide_border=true" />
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=CaptainRhett&layout=compact&theme=default&hide_border=true" />
</p>

---

## Contact

Please feel free to contact me if you are interested in my work.

- Email: `captainrhettyao#gmail.com`
- GitHub: [CaptainRhett](https://github.com/CaptainRhett)